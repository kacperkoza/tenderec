...
def _filter_by_deadline(tenders: list[dict]) -> list[dict]:
    """Odfiltruj przetargi, na które jeszcze można zdążyć (deadline > dziś)."""
...
def _save_result(result: dict) -> None:
    """Zapisuje wynik klasyfikacji do pliku organizations_by_industry.json."""
...
def _load_from_file() -> dict:
    """Odczytuje wcześniej zapisaną klasyfikację z pliku."""
    with open(constants.INDUSTRIES_OUTPUT_PATH, "r", encoding="utf-8") as f:
        return json.load(f)


def _classify_via_llm() -> dict:
    """Klasyfikuje organizacje przez LLM i zapisuje wynik do pliku."""
...
def get_industries() -> dict:
    """
    Zwraca klasyfikację organizacji wg branży.
